...
The rest of few variables are a little bit tricky here. For `date`, it makes little sense to know on which date arrests occurred but instead, knowing the months could provide some insights. Since `date` is in its own type (by default, R makes all date/time-related variables in POSIXIt format), we use functions from the `lubridate` package to make the transformation.
...
#| fig-cap: Blacks were 8 more times more likely to be arrested than other racial groups in 2021
...
#  scale_color_discrete(name="") +
  ...
  ggtext::geom_richtext(aes(x = 10, y = 360,
    label = "Racial divide in arrest numbers <br>expanded in the second half of 2021.<br> Between August and December,<br>  over <span style='color:darkred'>**600**</span> African Americans<br> were arrested, while the <br>number for Whites was just <br>slightly over 70, an eight-time gap."),
...
#| fig-cap: Over 2000 people were arrested for assaults or intimidation charges in 2022, the highest offense group.
